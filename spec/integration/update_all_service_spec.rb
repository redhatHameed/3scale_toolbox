...
RSpec.shared_context :update_all_stubbed_external_http_client do
  ...
  before :example do
    ...
    expect(external_target_client).to receive(:post).exactly(3).times.with('/admin/api/services/100/metrics/1/methods', anything).and_return({})
    expect(external_source_client).to receive(:post).exactly(4).times.with('/admin/api/services/1/metrics', anything).and_return({})
    expect(external_target_client).to receive(:post).exactly(4).times.with('/admin/api/services/100/metrics', anything).and_return({})
    ...
    expect(external_target_client).to receive(:post).exactly(8).times.with('/admin/api/application_plans/1/metrics/1/limits', anything).and_return({})
...
RSpec.shared_context :update_all_stubbed_internal_http_client do
  ...
  before :example do
    ...
    expect(internal_http_client).to receive(:put).with('/admin/api/services/100', internal_expected_target_service).and_return(internal_target_service)
    # get source settings
...
RSpec.shared_context :update_all_stubbed_api3scale_clients do
  include_context :update_all_stubbed_internal_http_client
  include_context :update_all_stubbed_external_http_client
...
RSpec.describe 'Update Service' do
  if ENV.key?('ENDPOINT')
    ...
    include_context :update_all_stubbed_api3scale_clients
  end
